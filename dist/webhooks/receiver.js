...
app.post('/webhook', (req, res) => {
    ...
    try {
        ...
        if (event === 'payment.failed') {
          ...
          `).run(caseId, payment.id, payment.subscription_id || null, payment.invoice_id || null, payment.customer_id || 'unknown_customer', payment.amount, payment.currency || 'INR', payment.error_code || null, now, now);
                    db.prepare(`
        ...
        `).run(crypto.randomUUID(), caseId, payment.id, payment.amount, payment.method || 'unknown', caseId ? 1 : 0, now);
                if (caseId) {
            ...
            if (sub) {
                const existingCase = db.prepare('SELECT id FROM cases WHERE subscription_id = ? AND status != ?').get(sub.id, 'closed');
                if (existingCase) {
                    db.prepare(`
            INSERT INTO audit_log (id, case_id, event_type, actor, payload, created_at)
