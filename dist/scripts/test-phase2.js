...
async function main() {
  ...
  `).run(caseId, paymentId, new Date().toISOString(), new Date().toISOString());
    ...
    // Verify DB updated
    let caseRow = db.prepare('SELECT failure_category FROM cases WHERE id = ?').get(caseId);
    ...
    // Verify DB updated
    caseRow = db.prepare('SELECT recovery_probability, expected_recovery_value FROM cases WHERE id = ?').get(caseId);
    ...
    // Verify DB updated
    caseRow = db.prepare('SELECT recommended_action FROM cases WHERE id = ?').get(caseId);
