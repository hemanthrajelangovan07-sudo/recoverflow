...
server.listen(0, async () => {
    ...
    try {
        ...
        // Verify DB
        const caseRow = db.prepare('SELECT * FROM cases WHERE source_payment_id = ?').get('pay_Test123');
        ...
        if (caseRow) {
            ...
            const updatedCase = db.prepare('SELECT status FROM cases WHERE id = ?').get(caseRow.id);
            console.log('Case status updated to:', updatedCase.status);
            const paymentRow = db.prepare('SELECT * FROM payments WHERE razorpay_payment_id = ?').get('pay_Rec123');
