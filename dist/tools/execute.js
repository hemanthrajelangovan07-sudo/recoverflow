...
function logAudit(caseId, eventType, payload) {
    db.prepare(`
    INSERT INTO audit_log (id, case_id, event_type, actor, payload, created_at)
    VALUES (?, ?, ?, 'agent', ?, ?)
  `).run(crypto.randomUUID(), caseId, eventType, JSON.stringify(payload), new Date().toISOString());
}
function logRejection(caseId, actionType, rejectedResult) {
    logAudit(caseId, 'policy_rejected', { action: actionType, ...rejectedResult });
}
export async function createPaymentLink(case_id) {
...
export async function issueInvoice(case_id) {
...
export async function sendRecoveryMessage(case_id, channel) {
...
export async function scheduleRetry(case_id, timestamp) {
    const rejected = checkAction('schedule_retry', case_id);
    if (rejected) {
        logRejection(case_id, 'schedule_retry', rejected);
        return rejected;
    }
    const caseRow = db.prepare('SELECT attempts FROM cases WHERE id = ?').get(case_id);
...
export async function stopRecovery(case_id, reason) {
