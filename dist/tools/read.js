...
export async function getPaymentFailure(payment_id) {
    try {
...
export async function getRecoveryStatus(case_id) {
    const caseRow = db.prepare('SELECT status, attempts, messages_sent FROM cases WHERE id = ?').get(case_id);
    if (!caseRow) {
        throw new Error(`Case ${case_id} not found`);
    }
    // Calculate amount recovered if captured
    const paymentRow = db.prepare('SELECT SUM(amount) as total FROM payments WHERE case_id = ? AND status = ?').get(case_id, 'captured');
