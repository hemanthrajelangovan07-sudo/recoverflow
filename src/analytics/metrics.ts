...
export function getBatchSummary() {
  const cases = db.prepare('SELECT status, amount, opted_out, created_at, updated_at FROM cases').all() as any[];
  const payments = db.prepare('SELECT amount, status, is_recovery_payment FROM payments').all() as any[];
  const messages = db.prepare('SELECT id FROM messages').all() as any[];
  ...
  const interventions_executed = db.prepare(`SELECT count(*) as count FROM audit_log WHERE event_type IN ('payment_link_created', 'message_sent', 'invoice_issued', 'retry_scheduled')`).get() as any;
