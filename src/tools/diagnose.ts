...
export async function classifyFailure(payment_id: string) {
  const caseRow = db.prepare('SELECT id, failure_category FROM cases WHERE source_payment_id = ?').get(payment_id) as any;
  if (!caseRow) {
  ...
  if (code.includes('insufficient') || code.includes('balance')) {
    ...
    try {
      ...
      if (text.includes('insufficient') || text.includes('balance')) category = 'insufficient_funds';
      else if (text.includes('timeout') || text.includes('temporary')) category = 'temporary_timeout';
      else if (text.includes('expire')) category = 'expired_method';
      else if (text.includes('mandate_cancelled') || text.includes('cancelled')) category = 'mandate_cancelled';
      else if (text.includes('fraud') || text.includes('suspect')) category = 'suspected_fraud';
      else if (text.includes('abandon')) category = 'customer_abandoned';
    } catch(e) {
  ...
  `).run(
    crypto.randomUUID(),
    case_id,
    JSON.stringify({ category, confidence, tool: 'classify_failure' }),
...
export async function estimateRecoveryProbability(case_id: string) {
  const caseRow = db.prepare('SELECT amount, failure_category FROM cases WHERE id = ?').get(case_id) as any;
  if (!caseRow) {
  ...
  if (cat === 'temporary_timeout') probability = 0.9;
  else if (cat === 'insufficient_funds') probability = 0.6;
  else if (cat === 'expired_method') probability = 0.3;
  else if (cat === 'customer_abandoned') probability = 0.2;
  else if (cat === 'mandate_cancelled') probability = 0.1;
  else if (cat === 'hard_decline') probability = 0.05;
  else if (cat === 'suspected_fraud') probability = 0.0;
  ...
  `).run(
    crypto.randomUUID(),
    case_id,
    JSON.stringify({ probability, expected_amount, expected_recovery_value }),
    new Date().toISOString()
  );
...
export async function selectRecoveryAction(case_id: string) {
  const caseRow = db.prepare('SELECT amount, failure_category, recovery_probability FROM cases WHERE id = ?').get(case_id) as any;
  if (!caseRow) {
    throw new Error(`No case found for case_id: ${case_id}`);
  }
  ...
  `).run(
    crypto.randomUUID(),
    case_id,
    JSON.stringify({ recommended_action, rationale, requires_approval }),
    new Date().toISOString()
  );
