...
export function checkAction(actionType: string, caseId: string): RejectedResult | null {
  ...
  const row = db.prepare('SELECT config_json FROM policy_config WHERE id = 1').get() as any;
  if (!row) {
