...
async function main() {
  ...
  let auditRows = db.prepare('SELECT * FROM audit_log WHERE case_id = ?').all(caseId1) as any[];
  ...
  auditRows = db.prepare('SELECT * FROM audit_log WHERE case_id = ?').all(caseId2) as any[];
  ...
  auditRows = db.prepare('SELECT * FROM audit_log WHERE case_id = ?').all(caseId3) as any[];
