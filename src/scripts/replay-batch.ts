...
function simulateCaptureIfApplicable(caseId: string, probability: number) {
...
async function runBaseline(prefix: string) {
  console.log(`\n--- Running Baseline (${prefix}) ---`);
  const cases = db.prepare(`SELECT id, failure_category FROM cases WHERE source_payment_id LIKE ?`).all(`pay_${prefix}_%`) as any[];
...
async function runAgent(prefix: string) {
  console.log(`\n--- Running Agent (${prefix}) ---`);
  const cases = db.prepare(`SELECT id, source_payment_id FROM cases WHERE source_payment_id LIKE ?`).all(`pay_${prefix}_%`) as any[];
...
async function compareMetrics(baselinePrefix: string, agentPrefix: string) {
  console.log(`\n--- Metrics Comparison ---`);
  ...
  `).get(`pay_${baselinePrefix}_%`) as any;

  const agentPayments = db.prepare(`
    SELECT SUM(p.amount) as total 
    FROM payments p JOIN cases c ON p.case_id = c.id
    WHERE p.status = 'captured' AND c.source_payment_id LIKE ?
  `).get(`pay_${agentPrefix}_%`) as any;
