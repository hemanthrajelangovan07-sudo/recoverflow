...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    let result;
    switch (name) {
      case 'get_payment_failure': result = await getPaymentFailure((args as any).payment_id); break;
      case 'get_recovery_status': result = await getRecoveryStatus((args as any).case_id); break;
      case 'classify_failure': result = await classifyFailure((args as any).payment_id); break;
      case 'estimate_recovery_probability': result = await estimateRecoveryProbability((args as any).case_id); break;
      case 'select_recovery_action': result = await selectRecoveryAction((args as any).case_id); break;
      case 'create_payment_link': result = await createPaymentLink((args as any).case_id); break;
      case 'issue_invoice': result = await issueInvoice((args as any).case_id); break;
      case 'send_recovery_message': result = await sendRecoveryMessage((args as any).case_id, (args as any).channel); break;
      case 'schedule_retry': result = await scheduleRetry((args as any).case_id, (args as any).timestamp); break;
      case 'stop_recovery': result = await stopRecovery((args as any).case_id, (args as any).reason); break;
      default:
